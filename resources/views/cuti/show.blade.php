...
@section('content')
    {{-- Header --}}
    <x-ui.header :back="route(auth()->user()->routePrefix() . 'cuti.index')">Detail Cuti</x-ui.header>

    {{-- Breadcrumb --}}
    <x-ui.breadcrumb :breadcrumbs="Breadcrumbs::generate(auth()->user()->routePrefix() . 'cuti.show', $cuti)" />
    ...
    <section class="space-y-6">
        <x-ui.detail-section>
            ...
            <x-ui.detail-block title="Informasi Status Cuti">
                <div>
                    <dt class="text-sm font-medium text-gray-500 mb-1">Keputusan Atasan</dt>
                    <span
                        class="{{ \App\Constants\Cuti::STATUS_COLOR[$cuti->keputusan_atasan] . ' inline-flex items-center px-2 py-1 ring-1 ring-inset text-sm font-medium rounded w-fit' }}">{{ $cuti->keputusan_atasan }}</span>
                </div>

                <div>
                    <dt class="text-sm font-medium text-gray-500 mb-1">Status Cuti</dt>
                    <span
                        class="{{ \App\Constants\Cuti::STATUS_COLOR[$cuti->status_cuti] . ' inline-flex items-center px-2 py-1 ring-1 ring-inset text-sm font-medium rounded w-fit' }}">{{ $cuti->status_cuti }}</span>
                </div>
            </x-ui.detail-block>
            ...
            <div class="flex items-center flex-wrap justify-end max-xs:justify-start gap-3 pt-6 border-t border-gray-200">
                ...
                @if ($cuti->status_cuti == 'Perubahan' || auth()->user()->role === 'admin')
                    <a href="{{ route(auth()->user()->routePrefix() . 'cuti.edit', $cuti->id) }}"
                ...
                <x-ui.delete-button :action="route(auth()->user()->routePrefix() . 'cuti.destroy', $cuti)" title="Hapus" message="Apakah Anda yakin ingin menghapus data cuti?"
                    confirmText="Ya, hapus" cancelText="Tidak, batalkan" />
            </div>
        </x-ui.detail-section>
    </section>
@endsection
