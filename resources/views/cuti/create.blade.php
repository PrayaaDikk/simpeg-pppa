...
@section('content')
    ...
    @if (auth()->user()->role !== 'admin')
        <x-ui.header :back="route(auth()->user()->routePrefix() . 'cuti.index')">Tambah Kenaikan Gaji Berkala</x-ui.header>
    @else
        <x-ui.header :back="route(auth()->user()->routePrefix() . 'pegawai.show', $pegawai)">Tambah Kenaikan Gaji Berkala</x-ui.header>
    @endif

    {{-- BreadCrumb --}}
    <x-ui.breadcrumb :breadcrumbs="Breadcrumbs::generate(auth()->user()->routePrefix() . 'cuti.create', $pegawai->id)" />
