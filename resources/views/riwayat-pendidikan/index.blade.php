...
@section('content')
    {{-- Header --}}
    <x-ui.header :back="route(auth()->user()->routePrefix() . 'pegawai.show', $pegawaiId)">Riwayat Pendidikan</x-ui.header>

    {{-- BreadCrumb --}}
    <x-ui.breadcrumb :breadcrumbs="Breadcrumbs::generate(auth()->user()->routePrefix() . 'riwayat-pendidikan.index', $pegawaiId)" />
    ...
    <section>
        ...
        <div class="bg-white shadow-sm rounded-bl-lg rounded-br-lg ">
            ...
            <div class="ml-auto w-fit mb-2 p-6">
                ...
                {{-- Create Button --}}
                <a href="{{ route(auth()->user()->routePrefix() . 'riwayat-pendidikan.create', $pegawaiId) }}"
