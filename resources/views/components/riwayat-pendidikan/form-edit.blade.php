...
<form class="max-w-5xl mx-auto"
    action="{{ route(auth()->user()->routePrefix() . 'riwayat-pendidikan.update', $riwayatPendidikan->id) }}"
        ...
        @if ($riwayatPendidikan->ijazah)
            <div class="mb-3">
                <p class="text-sm text-gray-500 mt-1 hover:underline"><a
                        href="{{ route('riwayat-pendidikan.showFile', $riwayatPendidikan->ijazah) }}"
                        target="_blank">Lihat
                        file sebelumnya</a></p>
            </div>
        @endif
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route(auth()->user()->routePrefix() . 'riwayat-pendidikan.show', $riwayatPendidikan) }}"
