...
@section('content')
    {{-- Header --}}
    <x-ui.header :back="route(auth()->user()->routePrefix() . 'pegawai.index')">Detail Pegawai</x-ui.header>

    {{-- Breadcrumb --}}
    <x-ui.breadcrumb :breadcrumbs="Breadcrumbs::generate(auth()->user()->routePrefix() . 'pegawai.show', $pegawai)" />
        ...
        <x-ui.detail-section>
            @if (auth()->user()->role === 'admin')
                <div class="flex items-center justify-end gap-4 max-xs:justify-start">
                    <a href="{{ route(auth()->user()->routePrefix() . 'cuti.create', $pegawai->id) }}"
                    ...
                    <a href="{{ route(auth()->user()->routePrefix() . 'kgb.create', $pegawai->id) }}"
            ...
            <div class="flex items-center flex-wrap justify-end max-xs:justify-start gap-3 pt-6 border-t border-gray-200">
                <a href="{{ route(auth()->user()->routePrefix() . 'pegawai.edit', $pegawai->id) }}"
                ...
                @if (auth()->user()->role === 'admin')
                    <x-ui.delete-button :action="route('admin.pegawai.destroy', $pegawai)" title="Hapus Pegawai"
                        message="Apakah Anda yakin ingin menghapus data pegawai {{ $pegawai->nama }}?"
